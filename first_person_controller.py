...
class FirstPersonController(Entity):
    def __init__(self, height=2, **kwargs):
        self.cursor = Entity(parent=camera.ui, model='quad', color=color.pink, scale=0.01, rotation_z=45)
        super().__init__()
        self.speed = 5
        self.height = height
        ...
        # Webcam look control
        self.webcam_look_sensitivity = 2.0  # Adjust as needed
        self._webcam_look_vec = None

    def apply_webcam_look(self, direction_vec):
        '''Set the direction vector [yaw_delta, pitch_delta] from webcam. direction_vec = [yaw, pitch]'''
        self._webcam_look_vec = direction_vec
    ...
    def update(self):
        # Webcam look control (overrides mouse if present)
        if self._webcam_look_vec is not None:
            yaw_delta, pitch_delta = self._webcam_look_vec
            self.rotation_y += yaw_delta * self.webcam_look_sensitivity
            self.camera_pivot.rotation_x += pitch_delta * self.webcam_look_sensitivity
    ...
    def on_enable(self):
        mouse.locked = True
        mouse.position = Vec2.zero() # bring mouse back to center
